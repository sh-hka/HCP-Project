...
class Application(db.Model):
    ...
    def __dict__(self):
        return {'id': self.id, 'first_name': self.first_name, 'last_name': self.last_name,
                'initial_purchase': self.initial_purchase, 'address': self.address, 'housing_type': self.housing_type,
                'phone': self.phone, 'phone_type': self.phone_type, 'email': self.email, 'ssn': self.ssn,
                'dob': self.dob}
    ...
    def __repr__(self):
        return '<Application {id:02d}>\n' \
               ...
               '\t{email}\n' \
               'Addr:\n{addr}\n' \
               '</Application>'.format(id=self.id, first_name=self.first_name, last_name=self.last_name, phone=self.phone,
                                       phone_type=self.phone_type, email=self.email, addr=self.address)
